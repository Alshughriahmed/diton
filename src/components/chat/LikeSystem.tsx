...
setShowHeart(true);
    ...
    try {
      // Send to server using P6 API-first pattern
      ...
      if (result && result.ok && !result.duplicate) {
        // Success - re-fetch actual state from server
        const getResponse = await safeFetch(`/api/like?pairId=${encodeURIComponent(currentPairId)}`, {
          method: 'GET'
        });
        ...
        if (getResponse.ok) {
          ...
          setLikeData(prev => ({
            ...prev,
            peerLikes: serverData.count || 0,
            isLiked: serverData.you || false,
            canLike: true
          }));
          ...
          emit("ui:like", {
            isLiked: serverData.you || false,
            myLikes: serverData.count || 0,
            pairId: currentPairId
          });
        } else {
          // GET failed - keep optimistic state
          setLikeData(prev => ({ ...prev, canLike: true }));
        }
      } else if (result && result.duplicate) {
        // Duplicate request - just re-enable button
        console.log('[like] Duplicate request detected by idempotency');
        setLikeData(prev => ({ ...prev, canLike: true }));
      } else {
