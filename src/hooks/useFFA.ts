...
function sub(cb:()=>void){
  const h=()=>cb();
  try{ window.addEventListener("ffa:ready",h as any,{passive:true}); window.addEventListener("ditona:ffa",h as any,{passive:true}); }catch{}
  return ()=>{ try{ window.removeEventListener("ffa:ready",h as any); window.removeEventListener("ditona:ffa",h as any);}catch{} };
}
