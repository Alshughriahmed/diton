...
export default function PlansPage(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const r=await safeFetch("/api/stripe/prices",{cache:"no-store"});
  ...
  const buy=async(id:string)=>{
    try{
      const r=await safeFetch("/api/stripe/subscribe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({priceId:id})});
      const j=await r.json();
