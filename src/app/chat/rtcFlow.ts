...
type Phase = "searching" | "matched" | "idle" | "stopped" | "connected";
type PhaseSetter = (v: Phase | ((p: Phase) => Phase)) => void;
...
async function initAnon() {
  ...
  if (r?.ok) { try { const j = await r.json(); if (j?.anonId) setAnonId(String(j.anonId)); } catch {} }
}
...
async function pollMatchmake(signal: AbortSignal) {
  ...
  for (;;) {
    ...
    const res = await apiSafeFetch("/api/rtc/matchmake", { method: "GET", headers: rtcHeaders(), timeoutMs: 6000 }).catch(() => undefined);
    if (res?.status === 200) {
      try {
    ...
    if (res?.status === 400) await ensureEnqueue();
...
export async function start(
  ...
  if (typeof setPhase === "function") setPhase("matched");
  return m;
}
