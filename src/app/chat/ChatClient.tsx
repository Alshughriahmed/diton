...
export default function ChatClient() {
  ...
  async function prevReq(ticket: string): Promise<string | null> {
    ...
    const r = await fetch(`/api/match/prev?ticket=${encodeURIComponent(ticket)}`, { method: "GET", credentials: "include", cache: "no-store" });
    if (r.status === 204) return null;
    if (!r.ok) return null;
  ...
  async function leaveRoom(): Promise<void> {
    ...
    if (room) {
      try {
        ...
        for (const pub of pubs) { try { const tr: any = (pub as any).track; if (tr && typeof lp.unpublishTrack === "function") { await lp.unpublishTrack(tr, { stop: false }); } } catch {} }
      } catch {}
  ...
  function wireRoomEvents(room: Room, roomName: string, sid: number) {
    ...
    const onDisc = () => { if (!isActiveSid(sid)) return; dcDetach(); setPhase("stopped"); broadcastMediaState(); };
  ...
  useEffect(() => {
    ...
    // تبديل الكاميرا: يعمل دون اتصال. عند الاتصال يستبدل المسار.
                      ...
                      onClick={() => {
                        ...
                        initLocalMedia().then(async () => {
                          ...
                          if (localRef.current && s) {
                            (localRef.current as any).srcObject = s; localRef.current.muted = true; await safePlay(localRef.current); setReady(true);
                            broadcastMediaState();
