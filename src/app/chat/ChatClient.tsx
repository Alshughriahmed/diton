...
export default function ChatClient(){
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.addEventListener('ditona:peer-meta-ui', handleMetaUI as any);
      return () => window.removeEventListener('ditona:peer-meta-ui', handleMetaUI as any);
    }
  }, []);
  ...
  useEffect(()=>{
    ...
    let offUpsell=on("ui:upsell", (d:any)=>{
      if (ffa) return; // no redirect in FFA
      router.push(`/plans?ref=${d?.ref || d?.feature || 'generic'}`);
    });
    let offCountryFilter=on("filters:country", async (_value)=>{
      try {
    ...
    let offGenderFilterUpdate=on("filters:gender", async (_value)=>{
      try {
        const { useFilters } = await import('@/state/filters');
        const { gender, countries } = useFilters.getState();
    ...
    let offRtcPhase=on("rtc:phase" as any, (data)=>{
      setPhase(data.phase);
      setRtcPhase(data.phase); // keep compatibility
    });
    ...
    return ()=>{ 
      ...
      if (typeof window !== "undefined") {
        window.removeEventListener("ditona:peer-meta", handlePeerMeta as any);
      }
      unsubscribeMobile();
    };
  },[]);
  ...
  async function doMatch(_backward=false){
    // disabled legacy matcher
    return;
  }
                              ...
                              if (vip && typeof window !== "undefined") {
                                try {
                                  ...
                                  if (effects) {
                                    ...
                                    video.srcObject = s;
                                    void video.play();
                                    const processed = await effects.initialize(video);
                                    if (processed) {
