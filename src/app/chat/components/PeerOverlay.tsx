...
export default function PeerOverlay() {
  ...
  useEffect(() => {
    ...
    // مسح البطاقة عند مراحل البحث/الإقلاع
    const onPhase = (e: any) => {
    ...
    // مزامنة عداد الإعجابات (مع حارس الزوج)
    const onLikeSync = (e: any) => {
      ...
      if (typeof d.you === "boolean") {
        // يمكن استخدامه لإبراز زر محليًا، لكن HUD العلوي يعرض العدد فقط.
      }
    };
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-20">
      ...
      {/* أسفل يسار: البلد + المدينة + الجنس (رمز فقط بألوان ثابتة) */}
      <div className="absolute left-3 bottom-3 flex items-center gap-2">
        ...
        <span
          data-ui="peer-gender"
          className={`font-semibold leading-none text-[22px] md:text-[28px] ${genderColor}`}
        >
          {genderSym}
        </span>
      </div>
