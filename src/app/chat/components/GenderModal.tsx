...
const LAUNCH_OPEN = true; // no VIP limits in launch
...
export default function GenderModal({ open, onClose, selected, onChange }: Props) {
  ...
  const toggle = (key: GenderKey) => {
    ...
    if (s.has(key)) s.delete(key);
      ...
      s.add(key);
      const limit = LAUNCH_OPEN ? 2 : 1; // later: gate by VIP/FFA
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-2xl bg-white text-gray-900 shadow-xl pointer-events-auto"
