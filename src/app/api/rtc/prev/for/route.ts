...
export async function POST(req: NextRequest) {
  ...
  if (!anon) return __noStore(NextResponse.json({ error:"anon-required" }, { status:401 }));
  try{
    const body = await req.json().catch(()=>null) as any;
    const peer = String(body?.peer || "");
    if(!peer) return __noStore(NextResponse.json({ ok:false }, { status:400 }));
    ...
    return __noStore(NextResponse.json({ ok:false, error:String(e?.message||e).slice(0,140) }, { status:500 }));
  }
}
