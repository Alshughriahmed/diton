...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!pairId || !sdp) return withReqId(__noStore(NextResponse.json({ error: "bad-input" }, { status: 400 })));

    const role = await auth(anon, pairId);
    if (role !== "callee") return withReqId(__noStore(NextResponse.json({ error: "only-callee" }, { status: 403 })));

    const sdpStr = typeof sdp === "string" ? sdp : JSON.stringify(sdp);
    const ok = await setNxPx(`rtc:pair:${pairId}:answer`, sdpStr, 120_000);
    if (!ok) return withReqId(__noStore(NextResponse.json({ error: "exists" }, { status: 409 })));

    await expire(`rtc:pair:${pairId}`, 150);
    return __noStore(new NextResponse(null, { status: 204 }));
  } catch (e: any) {
    return withReqId(__noStore(NextResponse.json({ error: "answer-fail", info: String(e?.message || e).slice(0, 140) }, { status: 500 })));
  }
}
...
export async function GET(req: NextRequest) {
  ...
  await expire(`rtc:pair:${pairId}`, 150);
  return withReqId(__noStore(NextResponse.json({ sdp: String(sdp) }, { status: 200 })));
}
