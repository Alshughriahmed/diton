...
const GRACE_MS = (process.env.ICE_GRACE ?? "1") === "0" ? 0 : 5000; // ≤5s

function sizeOk(candidate: unknown): boolean {
  try {
...
export async function POST(req: NextRequest) {
  ...
  const roleHdr = req.headers.get("x-role");
  const b: any = await req.json().catch(() => ({}));

  const pairId = String(b?.pairId || fromHdr || fromQuery || "").trim();
  const role = String(b?.role || roleHdr || "").trim() as "caller" | "callee";
  ...
  const dstField = myRole === "caller" ? "ice_b" : "ice_a"; // push to peer
...
export async function GET(req: NextRequest) {
  ...
  if (!role) {
    ...
    const graceApplied =
      GRACE_MS > 0 && lastStopTs > 0 && Date.now() - lastStopTs <= GRACE_MS;

    logRTC({
      route: "/api/rtc/ice",
      status: graceApplied ? 204 : 403,
      rid: req.headers.get("x-req-id"),
      anonId: anon,
      pairId,
      phase: "grace-check",
