...
export async function POST(req: NextRequest) {
  ...
  if (authSecret !== expectedSecret) {
    return withReqId(NextResponse.json({ error: "Unauthorized" }, { status: 401 }));
  }
  ...
  try {
    ...
    switch (action) {
      ...
      default:
        return withReqId(NextResponse.json({ error: "Invalid action" }, { status: 400 }));
    }
    
  } catch (error) {
    console.error("[RECONCILE_ERROR]", error);
    return withReqId(NextResponse.json({ error: "Processing failed" }, { status: 500 }));
  }
}
