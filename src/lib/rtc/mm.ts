let __wroteRtcLast=false; function __setRtcLastOnce(k:string,ms:number){try{ if(__wroteRtcLast) return; __wroteRtcLast=true; setPx(k,ms);}catch{}}
import {
...
export async function matchmake(self:string){
  ...
  try{
            ...
            if(okA && okB){
              if(await setNxPx(`rtc:claim:${cand}`,self,6000)){
                ...
                if(await setNxPx(pairLock,"1",6000)){
                  ...
                  ,
        __setRtcLastOnce(`rtc:last:${self}`,  cand, 90_000), __setRtcLastOnce(`rtc:last:${cand}`,  self, 90_000)
      ]);
      ...
      await Promise.all([ 
        __setRtcLastOnce(`rtc:last:${self}`,  cand, 90_000), __setRtcLastOnce(`rtc:last:${selfBase}`,  candBase, 90_000),
        __setRtcLastOnce(`rtc:last:${cand}`,  self, 90_000), __setRtcLastOnce(`rtc:last:${candBase}`,  selfBase, 90_000)
      ]);
                  await Promise.all([
                    __setRtcLastOnce(`rtc:last:${self}`,  cand, 90_000), __setRtcLastOnce(`rtc:last:${cand}`,  self, 90_000)
                  ]);
