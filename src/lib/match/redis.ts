...
export type Entry = {
  ...
  selfCountry: string | null;         // ISO-2 if known
  filterGenders: Gender;
  filterCountries: string[];          // [] => ALL
  ts: number;
};

const TTL_SEC = parseInt(process.env.MATCH_TTL_SEC || "", 10) || 45; // 30–60s acceptable
...
const KATTR = (t: string) => `mq:attr:${t}`;        // HASH/JSON per ticket
const KROOM = (t: string) => `mq:room:${t}`;        // room name per ticket
const KDEV = (d: string) => `mq:dev:${d}`;          // deviceId -> ticket (optional helper)

// Utilities
function now() { return Date.now(); }
function parseJSON<T>(s: any): T | null { try { return s ? JSON.parse(String(s)) as T : null; } catch { return null; } }

// Deterministic room name for two tickets
...
export async function tryMatch(ticket: string, widen: boolean): Promise<{ room?: string } | null> {
  ...
  // purge old heads by score (older than TTL)
  ...
  // scan FIFO head
  const head = await rc(["ZRANGE", KQ, "0", "199"]); // up to 200 candidates
  ...
  for (const cand of candidates) {
    ...
    // set rooms with TTL, remove both from queue
    await rcp([
  ...
  // ensure I am enqueued at head with fresh ts
...
export async function purge(): Promise<number> {
  ...
  // also sweep tickets with missing attr among recent head
