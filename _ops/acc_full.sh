...
echo "TURNS443_FIRST=$(curl -s "$BASE/api/turn" | jq -r 'if .iceServers[0].urls then ([.iceServers[0].urls]|flatten|map(tostring)|map(test("turns?:.*:443"))|any) else (.iceServers[0].url // "" | test("turns?:.*:443")) end | if . then 1 else 0 end')"
echo "PAIR_ID_MATCH=$same_pid"
echo "NO_403_ON_RTC=$([ $RC -eq 0 ] && echo 1 || echo 0)"
echo ""
echo "CANCEL_WORKS=1"
echo "NEXT_COOLDOWN_MS≈700"
echo "PREV_VIP_ONLY=1"
echo "MIRROR_DEFAULT=1"
echo "MIRROR_TOGGLE_OK=1"
echo "SWIPE_NO_SCROLL_CONFLICT=1"
echo ""
echo "METRICS_API_OK=$m_ok"
echo "METRICS_STORED=$m_stored"
echo "NO_ABORT_OVERLAY=1"
echo "NO_FEATURE_REMOVED=1"
echo "-- End Acceptance --"