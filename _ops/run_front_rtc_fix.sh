...
safe_exec "pnpm build" | tee "$BUILD_LOG" >/dev/null
...
########################################
{
  echo "=== DitonaChat Front+RTC Fix Report ==="
  echo "Timestamp: $(date -u)"
  echo "Branch: $(git branch --show-current 2>/dev/null || echo 'unknown')"
  echo "Commit: $(git rev-parse --short HEAD 2>/dev/null || echo 'unknown')"
  echo
  echo "P1 - Pre-check results:"
  echo "  Server FFA: $SRV_FFA"
  echo "  Public FFA: $PUB_FFA"
  echo "  Age allow A/B: $A_AGE/$B_AGE"
  echo "  RTC init A/B: $A_INIT/$B_INIT"
  echo
  echo "P2 - FFA Runtime Hook:"
  echo "  ✅ Created src/hooks/useFFA.ts"
  echo "  ✅ Updated src/utils/ffa.ts"
  echo "  ✅ Updated ChatToolbar.tsx to use useFFA()"
  echo
  echo "P3 - Events Bridge:"
  echo "  ✅ Created freeForAllBridge.ts"
  echo "  ✅ Integrated with ChatClient.tsx"
  echo
  echo "P5 - Message Allow FFA Fast-path:"
  echo "  ✅ Updated /api/message/allow/route.ts"
  echo
  echo "Build Status:"
  ...
  echo
  echo "Files modified:"
  find src -name "*.ts" -o -name "*.tsx" | xargs ls -la --time-style=long-iso | awk '$6 >= "'$(date +%Y-%m-%d)'"' | head -10
  echo
  echo "Backup location: $BKP"
  echo "Temp files: $TDIR"
} > "$RPT"

say "Report written to: $RPT"
say "Backup location: $BKP"
echo "Front+RTC fix completed successfully!"